...
backSubmit.onclick=function(){
  let backDropdown = backDropdown.value
  let backSets = backSets.value
  let backReps = backReps.value
  let backWeight = backWeight.value
...
bicepsSubmit.onclick=function(){
  let bicepsDropdown = bicepsDropdown.value
  let bicepsSets = bicepsSets.value
  let bicepsReps = bicepsReps.value
  let bicepsWeight = bicepsWeight.value
  // ATTENTION NEED TO FIX: let query = "INSERT into exercise(name, reps, sets) SELECT exercise_group_id, 'curls', 10, 3 FROM exercise_group EG INNER JOIN workout w ON EG.workout_id = w.workout_idWHERE user_id = 1 AND w.`date`= '04-13-2021';
}
